...
2. **Do not allow random spaghetti growth in existing code.**
   ...
   - If a change adds weird conditionals in random places, treat that as a design problem, not a stylistic nit.
...
5. **Push hard on type and boundary cleanliness when they affect maintainability.**
   ...
   - If a branch relies on silent fallback to paper over an unclear invariant, ask whether the boundary should be explicit instead.
...
7. **Treat unnecessary sequential orchestration and non-atomic updates as design smells when the cleaner structure is obvious.**
   ...
   - Do not over-index on micro-optimizations, but flag avoidable orchestration complexity that makes implementation more brittle.
...
- Generic magical handling that hides simple structure and makes the code harder to reason about.
...
- the PR preserves a lot of incidental complexity when there is a plausible structural move that would delete it
